...
oos_sentence = [i[0] for i in oos_data]
oos_label = ['oos' for i in oos_data]
oos_sentence_train, oos_sentence_test, oos_label_train, oos_label_test = train_test_split(oos_sentence, oos_label, train_size=0.8)
...
X_train = X_train + oos_sentence_train
y_train = y_train + oos_label_train
...
for i in range(len(softmax_in_scope)):
    ...
    # confidence2 is the confidence of each prediction to be OOS
...
hn_sentence_test = [i[0] for i in hn_oos]
hn_label_test = ['oos' for i in hn_oos]
...
oos_text_encoded = tokenizer(oos_sentence_test, padding=True, truncation=True, return_tensors='tf')
oos_text_to_tensor = oos_text_encoded['input_ids']
encoded_oos_labels = [label_dict[label] for label in oos_label_test]
...
print("Accuracy oos from " + dataset_name + ": ", accuracy_oos)
...
file_name = "results_"+ model_name + "_" + dataset_name + "_oos_training.json"
